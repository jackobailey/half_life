...
Figure 1 shows the three steps that I take to estimate daily UK GDP. Each point in the left-most panel reflects a single estimate from the ONS' monthly time series of UK GDP. These begin in `r gdp$date %>% min() %>% format("%B %Y")`, end in `r gdp$date %>% max() %>% format("%B %Y")`, and are indexed such that July 2016 equals 100. Three patterns are most obvious. First, that UK GDP has tended to grow from one month to the next. Second, that the global financial crisis interrupted this process in the mid-2000s. And third, that GDP growth was faster before than after the financial crisis.
