...
We can now substitute this equation into our previous one to arrive at the final model that we will fit to the data. Note that in doing so we also include a set of covariates, $x$, that account for the time trend, the different Prime Ministers in power throughout the period, their time in office, and the interaction between each Prime Minister and their time in office. Finally, we also include a set of prior distributions on each of our parameters. This gives the following model:
