...
# Supplement 5: Logistic Replication
...
# economic vote decay according to a logistic and not an exponential curve.
# This allows the economic vote to plateau for a little before any decay
# begins.
  ...
  brm(
    formula = 
      bf(
        ...
        nlf(betaT ~ beta0*inv_logit(lambda*(mu - time))),
        beta0 + lambda ~ 1,
        nl = TRUE,
        decomp = "QR"
      ),
    ...
    prior =
      ...
      prior(normal(2.5, 2.5), nlpar = "lambda"),
    data = cms,
    iter = 2000,
    refresh = 5,
    chains = 4,
...
save_info(here("_output", "_session_info", "s005_logistic.txt"))
