...
# Supplement 6: Censored Linear Replication
...
# We're going to fit a model similar to the main model, but that has the
# economic vote decay in a linear manner, but that also ensures that any
# values of beta_t must not cross the x-axis. I.e. if they start positive
# they fall linearly to zero and stay there and if they start negative they
# increase linearly to zero and stay there.
  ...
  brm(
    formula = 
      bf(
        ...
        nlf(betaT ~ (beta0 + lambda*time) * step((-beta0/lambda) - time)),
        beta0 + lambda ~ 1,
        nl = TRUE,
        decomp = "QR"
      ),
...
save_info(here("_output", "_session_info", "s006_linear.txt"))
